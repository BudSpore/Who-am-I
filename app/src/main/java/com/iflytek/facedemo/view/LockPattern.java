package com.iflytek.facedemo.view;
...
public class LockPattern extends View
{
	...
	private int pwdMaxLen = 1000;// 密码最大长度
	...
	{
		String namespace = "http://schemas.android.com/apk/res/com.iflytek.facedemo";
	...
	public boolean onTouchEvent(MotionEvent event)
	{
		...
		if (isChecking && p != null)
		{
			...
			if (isCheck(p))
			{
				String A= Integer.toString(p.value);
				Log.d("chang", A);
		...
		if (!isChecking && mCompleteListener != null)
		{
			...
			{
				String A= Integer.toString(selectedPoints.size());
				Log.d("chang", A);
...
//		return false;
	...
	private void init()
	{
		...
		int avgCount = 4* side +7 ;
		//float spacing= (float) 80.;
...
//		for (int i = 0; i < mPoints.length; i++)
//		{
//			float y = offsetY +15+ (3 * i + 2) * spacing;
//			for (int j = 0; j < mPoints[i].length; j++)
//			{
//				float x = offsetX+30  + (3 * j + 2) * spacing;
//				mPoints[i][j] = new Point(x, y, value++);
//			}
//		}
		//菱形
//		y = offsetY +15+ (3 * 0 + 2) * spacing;
//		x= offsetX +30+ (3 * 2 + 2) * spacing;
//		mPoints[0][0] = new Point(x, y, 0);
//		y = offsetY +15+ (3 * 1 + 2) * spacing;
//		x= offsetX+ 30+(3 * 1 + 2) * spacing;
//		mPoints[0][1] = new Point(x, y, 1);
//		y = offsetY +15+ (3 * 1 + 2) * spacing;
//		x= offsetX+30+ (3 * 3 + 2) * spacing;
//		mPoints[0][2] = new Point(x, y, 2);
//		y = offsetY +15+ (3 * 2 + 2) * spacing;
//		x= offsetX+ 30+(3 * 0 + 2) * spacing;
//		mPoints[1][0] = new Point(x, y, 3);
//		y = offsetY +15+ (3 * 2 + 2) * spacing;
//		x= offsetX+ 30+(3 * 2 + 2) * spacing;
//		mPoints[1][1] = new Point(x, y, 4);
//		y = offsetY +15+ (3 * 2 + 2) * spacing;
//		x= offsetX+ 30+(3 * 4 + 2) * spacing;
//		mPoints[1][2] = new Point(x, y, 5);
//		y = offsetY +20+ (3 * 3 + 2) * spacing;
//		x= offsetX+ 30+(3 * 1 + 2) * spacing;
//		mPoints[2][0] = new Point(x, y, 6);
//		y = offsetY +15+ (3 * 3 + 2) * spacing;
//		x= offsetX+ 30+(3 * 3 + 2) * spacing;
//		mPoints[2][1] = new Point(x, y, 7);
//		y = offsetY +15+ (3 * 4 + 2) * spacing;
//		x= offsetX+ 30+(3 * 2 + 2) * spacing;
//		mPoints[2][2] = new Point(x, y, 8);
		//心形
		float z=(float)5;
		Log.d("lalal",Float.toString(offsetX));
		Log.d("lalall",Float.toString(offsetY));
		y = offsetY +40+ (3 * 2 + 2) * spacing;
		x= offsetX + (3 * z + 2) * spacing;
		mPoints[0][0] = new Point(x, y, 0);
		y = offsetY- 50+ (3 * 1 + 2) * spacing;
		x= offsetX+80+(3 * 1 + 2) * spacing;
		mPoints[0][1] = new Point(x, y, 1);
		y = offsetY - 50+ (3 * 1 + 2) * spacing;
		x= offsetX+50+ (3 * 3 + 2) * spacing;
		mPoints[0][2] = new Point(x, y, 2);
		y = offsetY +40+ (3 * 2 + 2) * spacing;
		x= offsetX+(3 * 0 + 2) * spacing;
		mPoints[1][0] = new Point(x, y, 3);
		y = offsetY +40+ (3 * 2+ 2) * spacing;
		x= offsetX+80+(3 * 1 + 2) * spacing;
		mPoints[1][1] = new Point(x, y, 4);
		...
		x= offsetX+50+(3 * 3+ 2) * spacing;
		mPoints[1][2] = new Point(x, y, 5);
		y = offsetY +120+ (3 * 3 + 2) * spacing;
		x= offsetX+ 80+(3 * 1 + 2) * spacing;
		mPoints[2][0] = new Point(x, y, 6);
		y = offsetY +120+ (3 * 3 + 2) * spacing;
		x= offsetX+ 50+(3 * 3 + 2) * spacing;
		mPoints[2][1] = new Point(x, y, 7);
		y = offsetY +180+ (3 * 4 + 2) * spacing;
		x= offsetX+ 60+(3 * 2 + 2) * spacing;
		mPoints[2][2] = new Point(x, y, 8);
		//float spacing= (float) 80.;
		avgCount = 3* side +1 ;
	    spacing = viewWidth / avgCount;
		outerRadius = spacing;
	...
	private void initPaints()
	{
		...
		this.linePaint.setStrokeWidth(this.outerRadius / 16);
...
class Point
{
	...
	public static final int STATE_CHECK_AGAIN = 3;
	 ...
	 * 该点当前状态
	 */
	public int jianyan= STATE_NORMAL;
